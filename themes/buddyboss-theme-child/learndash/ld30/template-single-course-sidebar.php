...
?>
<style>
    /* Course sidebar (sticked to the side) */
...
/* div > div.bb-grid > div.bb-single-course-sidebar.bb-preview-wrap > .bb-ld-sticky-sidebar{
			...
			<div class="bb-course-preview-content">
				...
				<div class="bb-course-status-wrap">
					...
					if ( class_exists( 'LearnDash\Core\Models\Product' ) ) {
						...
						if ( is_user_logged_in() && isset( $has_access ) && $has_access ) {
							...
							echo '<div><div class="ld-status ld-status-incomplete ld-third-background">' . __( 'Not Enrolled', 'buddyboss-theme' ) . '</div></div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						}
					}
				...
				<div class="bb-button-wrap">
					...
					if ( empty( $course_progress ) && 100 > $course_progress ) {
						...
						$btn_advance_label = sprintf( __( 'התחל %s', 'buddyboss-theme' ), LearnDash_Custom_Label::get_label( 'course' ) );
						...
						$btn_advance_label = __( 'הושלם', 'buddyboss-theme' );
					} else {
						$btn_advance_class = 'btn-advance-continue';
						$btn_advance_label = __( 'המשך', 'buddyboss-theme' );
						$resume_link       = buddyboss_theme()->learndash_helper()->boss_theme_course_resume( $course_id );
					}
				...
				if ( 0 < sizeof( $lesson_count ) || 0 < $topics_count || 0 < $course_quizzes_count || $course_certificate ) {
					...
					<h4><?php echo sprintf( esc_html__( 'ה%s כולל:', 'buddyboss-theme' ), $course_label ); ?></h4>
